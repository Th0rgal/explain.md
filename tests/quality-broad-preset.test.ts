...
describe("quality broad preset", () => {
  it("produces deterministic broad-corpus pressure within configured bounds", async () => {
    const preset = resolveQualityBenchmarkPreset("fixture-verity-broad");
    ...
    expect(report.metrics.parentCount).toBeGreaterThanOrEqual(4);
    expect(report.repartitionMetrics.eventCount).toBeGreaterThan(0);
    expect(report.repartitionMetrics.maxRound).toBeLessThanOrEqual(1);
