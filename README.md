...
- Issue #12 follow-up: cache invalidation is theorem-delta aware (`cache_semantic_hit` for source-only edits), with deterministic affected-ancestor subtree recompute on statement-only deltas (`cache_incremental_subtree_rebuild`) and deterministic topology-aware rebuild with auditable stable-id, same-depth child-hash, and same-depth child-statement-hash parent-summary reuse on topology/structure deltas (`cache_incremental_topology_rebuild`, fallback `cache_incremental_rebuild`, ambiguity skips tracked in diagnostics).
