...
- Cache report route exposes deterministic cache-reuse diagnostics (`status`, `cacheKey`, `sourceFingerprint`, `snapshotHash`, `cacheEntryHash`) and theorem-delta-aware outcomes (`cache_semantic_hit`, `cache_incremental_subtree_rebuild`, `cache_incremental_topology_rebuild`, `cache_incremental_rebuild`) with auditable topology reuse counters (stable-id reuse, same-depth child-hash reuse, and ambiguous child-hash skip) for reproducible incremental recompute auditing.
