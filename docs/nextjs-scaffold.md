...
- Proof Explorer control panel exposes `entailmentMode` directly so strict lexical-entailment runs can be triggered and audited from browser routes.
  - `entailmentMode=strict` is deterministic fail-closed:
    - parent evidence refs must cover every grouped child
    - no newly introduced terms are permitted
    - unsupported-term checks include both parent statement and rationale text
...
- Deterministic large-tree render benchmark is available via `npm run web:bench:tree-scale` (writes `docs/benchmarks/tree-scale-evaluation.json`).
- CI enforces large-tree render benchmark reproducibility via `npm run web:eval:tree-scale:ci` (baseline-compares request/outcome hashes and emits `.explain-md/tree-scale-evaluation-report.json`).
- Deterministic explanation-diff benchmark is available via `npm run web:bench:explanation-diff` (writes `docs/benchmarks/explanation-diff-evaluation.json`).
...
- Root release gate composes quality baseline, summary-security benchmark, and web benchmark evidence via `npm run eval:release-gate:ci` (emits `.explain-md/release-gate-report.json`).
