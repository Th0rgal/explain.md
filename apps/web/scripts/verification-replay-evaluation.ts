...
async function main(): Promise<void> {
  ...
  if (options.baselinePath) {
    ...
    const baseline = JSON.parse(baselineRaw) as ReturnType<typeof runVerificationReplayEvaluation>;
    assertVerificationReplayEvaluationBaseline(baseline, report);
  }
  ...
  if (options.outPath) {
    ...
    console.log(`Wrote verification replay evaluation report to ${resolvedPath}`);
    return;
  }
