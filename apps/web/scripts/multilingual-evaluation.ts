...
async function main(): Promise<void> {
  ...
  if (options.baselinePath) {
    ...
    const baseline = JSON.parse(baselineRaw) as Awaited<ReturnType<typeof runMultilingualEvaluation>>;
  ...
  if (options.outPath) {
    ...
    console.log(`Wrote multilingual evaluation report to ${resolvedPath}`);
    return;
  }
