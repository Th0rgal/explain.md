...
- Query deterministic cache reuse diagnostics with `/api/proofs/cache-report` (`status`, `cacheKey`, `sourceFingerprint`, `snapshotHash`, `cacheEntryHash`, plus theorem-delta-aware `cache_semantic_hit`/`cache_incremental_subtree_rebuild`/`cache_incremental_topology_rebuild`/`cache_incremental_rebuild` codes and topology reuse counters for stable-id reuse, same-depth child-hash reuse, same-depth child-statement-hash reuse, and ambiguity-skip counters).
